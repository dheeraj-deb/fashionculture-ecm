...
exports.incQty = (req, res) => {
  product
    .incQtyAndTotal(req.session.user._id, req.body)
    .then((data) => {
      if (data) {
        productTotal = data;
        res.json({
...
exports.placeOrder = async (req, res) => {
  ...
  const discount_price = await product.isDiscountAvailable(
    req.session.user._id
  );
  const cartTotal = await product.getCartTotal(req.session.user._id);
  const address = await user.getAddressByAddressId(
    req.body.address[1],
    req.session.user._id
  );
  if (discount_price) {
...
exports.editAddress = async (req, res) => {
  ...
  const response = await user.editAddress(
    addrId,
    req.body,
    req.session.user._id
  );
...
exports.changeAddress = async (req, res) => {
  console.log(req.body);
  const addr = await user.getAddressByAddressId(
    req.body.address,
    req.session.user._id
  );
  res.json(addr);
...
exports.add_to_whishlist = async (req, res) => {
  ...
  const response = await product.addWhishlist(
    req.session.user._id,
    req.body.product
  );
  console.log("success");
...
exports.getMyOrders = async (req, res, next) => {
  try {
    let orderDetails = await user.findOrders(req.session.user._id);
    res.render("user/my-profile-orders", {
      layout: "user-layout",
      order: true,
      user: true,
      profile: true,
  ...
  if (response) {
    ...
    transporter.sendMail({
      ...
      html: `
        ...
        } Your Password Is Changed Succesfully</p>`,
    });
  }
};

exports.getForgotPass = async (req, res) => {
  res.render("user/my-profile-forgotpass", {
    layout: "user-layout",
    user: true,
    profile: true,
    myprofile: true,
...
exports.validatePass = async (req, res) => {
  console.log(req.body);
  const response = await user.validatePassword(
    req.body.password,
    req.session.user._id
  );
  if (!response) {
...
exports.create_coupon_discount = async (req, res) => {
  const disObj = await product.create_coupon_discount(
    req.body.coupon,
    req.session.user._id
  );
  res.json(disObj);
