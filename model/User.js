...
exports.placeOrder = (
  ...
  return new Promise(async (resolve, reject) => {
    ...
    coupon_discount < cartTotal
      ? (discountedAmd = coupon_discount)
      : discountedAmd;
    const orderObj = {
    ...
    const order = await db
      ...
      .aggregate([
        ...
        },
        {
          $addFields: {
            discount: {
              $subtract: ["$orderObj.totalAmount", "$orderObj.coupon_discount"],
            },
          },
        },
        {
...
exports.verifyPayment = (data) => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const expectedSignature = crypto
      .createHmac("sha256", "rU5Oo1OqZAgybktcAuxaOrwH")
...
exports.generateInvoice = (orderId) => {
  return new Promise(async (resolve, reject) => {
    const order = await db
      .get()
      .collection(collection.ORDER_COLLECTION)
      .findOne({ "orderObj.orderId": objectId(orderId) });
    if (order) {
